...
execute as @a[tag=playing,nbt={SelectedItem:{id:"minecraft:carrot"}}] unless score $track-67-timer music matches 0..362 unless entity @s[x=-4.5,y=38,z=-6.5,distance=..2] run effect give @s levitation 1 3 true
execute as @a[tag=playing,nbt=!{SelectedItem:{id:"minecraft:carrot"}}] unless score $track-67-timer music matches 0..362 run effect clear @s levitation
