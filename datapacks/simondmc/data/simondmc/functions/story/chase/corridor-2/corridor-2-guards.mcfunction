...
# animation
execute as @a[tag=playing] if predicate simondmc:corridor-2 unless score $corridor-anim-2 guards matches 0.. run scoreboard players set $corridor-anim-2 guards 0
...
execute if score $corridor-anim-2 guards matches 31 run setblock -40 69 32 minecraft:magenta_glazed_terracotta[facing=east]
# instakill if there for too long like just go down man
execute if score $corridor-anim-2 guards matches 150.. as @a[tag=playing] if predicate simondmc:corridor-2 run damage @s 100 mob_attack by @e[tag=corridor-2-guard,limit=1]
# drop down into underground corridor unless trapdoor closed
execute if score $corridor-anim-2 guards matches 150 run scoreboard players set $override guard-walk-anim 1
execute if score $corridor-anim-2 guards matches 150..230 unless score $closed trapdoor matches 1 as @e[tag=corridor-2-guard] run function simondmc:story/chase/corridor-2/move-forward
# make sure player can't just hang out right above the pit waiting for guards to pass
execute if score $corridor-anim-2 guards matches 200 unless score $closed trapdoor matches 1 as @a[tag=playing] if predicate simondmc:pit-1-tunnel run damage @s 100 mob_attack by @e[tag=corridor-2-guard,limit=1]
execute if score $corridor-anim-2 guards matches 230 run scoreboard players reset $override guard-walk-anim
# activate machine gun if guards fall down
execute if score $corridor-anim-2 guards matches 210 unless score $closed trapdoor matches 1 run scoreboard players set $machine-gun guards 1
execute if score $corridor-anim-2 guards matches 210 unless score $closed trapdoor matches 1 run scoreboard players set $SHOOT-PERIOD guards 3
