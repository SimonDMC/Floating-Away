...
execute as @a[x=-4.5,y=38,z=-6.5,distance=..1] unless score $end elevator matches 0.. run fill -3 38 -6 -3 40 -8 barrier
# moving pistons are so that you can't place blocks in there and start building inside of the elevator haha
execute as @a[x=-4.5,y=38,z=-6.5,distance=..1] unless score $end elevator matches 0.. run fill -3 39 -6 -3 39 -8 moving_piston
# close doors
...
execute if score $end elevator matches 50..362 as @a[x=-6.0,y=67.0,z=-8.0,dy=1,dx=3,dz=3] run fill -6 64 -8 -4 64 -6 minecraft:quartz_block
execute if score $end elevator matches 50..362 as @a[x=-6.0,y=67.0,z=-8.0,dy=1,dx=3,dz=3] run setblock -5 64 -7 minecraft:gold_block
# elevate blocks
