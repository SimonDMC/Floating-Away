...
execute if score $shoot-timer guards >= $SHOOT-PERIOD guards if entity @a[predicate=simondmc:corridor-1-shoot] as @e[tag=shooting-guard,tag=vent-guard,limit=1,sort=random] run function simondmc:story/chase/shoot-arrow
